...
public class SparkMaxSwerve extends SwerveMotor {
  ...
  public SparkMaxSwerve(CANSparkMax motor, boolean isDriveMotor, int moduleNumber) {
    ...
    // configureSparkMax(() -> motor.setCANTimeout(0)); // Commented out because it
    // prevents feedback.
  }

  /**
   * Initialize the {@link SwerveMotor} as a {@link CANSparkMax} connected to a
   * Brushless Motor.
   *
  ...
  public void configurePIDF(PIDFConfig config) {
    ...
    int pidSlot = 0;
    SmartDashboard.putNumber("p value: drive? " + isDriveMotor, config.p);
   ...
   * @param CANStatus5 Duty Cycle Absolute Encoder Position, Duty Cycle Absolute
   *                   Encoder Absolute Angle
   * @param CANStatus6 Duty Cycle Absolute Encoder Velocity, Duty Cycle Absolute
   *                   Encoder Frequency
   */
  ...
  public void setReference(double setpoint, double feedforward) {
    ...
    if (isDriveMotor) {
      configureSparkMax(() -> pid.setReference(
          setpoint,
