...
public class SwerveModule {
  ...
  /**
   * An {@link Alert} for if pushing the Absolute Encoder offset to the encoder
   * fails.
   */
  ...
  /**
   * Module number for kinematics, usually 0 to 3. front left -> front right ->
  ...
  public SwerveModule(int moduleNumber, SwerveModuleConfiguration moduleConfiguration,
    ...
    if (absoluteEncoder != null) {
      ...
      DriverStation.reportError(":3" + configuration.name + absoluteEncoder, new StackTraceElement[0]);
    }
    ...
    noEncoderWarning = new Alert("Motors", "There is no Absolute Encoder on module #" + moduleNumber,
  ...
  /**
   * Queue synchronization of the integrated angle encoder with the absolute
   * encoder.
   */
   ...
   * <b>WARNING: If you are not using one of the functions from
   * {@link SwerveDrive} you may screw up {@link SwerveDrive#kinematics}</b>
   *
   * @param desiredState Desired swerve module state.
   * @param isOpenLoop   Whether to use open loop (direct percent) or direct
   *                     velocity control.
   * @param force        Disables optimizations that prevent movement in the angle
   *                     motor and forces the desired state
  ...
  /**
   * Get the absolute position. Falls back to relative position on reading
   * failure.
   *
   ...
   *
   * @param conversionFactor Angle motor conversion factor for PID, should be
   *                         generated from
   ...
   *
   * @param conversionFactor Drive motor conversion factor for PID, should be
   *                         generated from
  ...
  /**
   * Fetch the {@link SwerveModuleConfiguration} for the {@link SwerveModule} with
  ...
  /**
   * Push absolute encoder offset in the memory of the encoder or controller. Also
   * removes the internal angle offset.
   */
  ...
  /**
   * Restore internal offset in YAGSL and either sets absolute encoder offset to 0
   * or restores old value.
   */
   ...
   *
   * @return If the last Absolute Encoder had a read issue, or absolute encoder
   *         does not exist.
   */
  ...
  public void updateTelemetry() {
    ...
    SmartDashboard.putNumber("Module[" + configuration.name + "] Adjusted Absolute Encoder", pos > 180 ? pos - 360.0 : pos);
    SmartDashboard.putNumber("Module[" + configuration.name + "] speed", driveMotor.getVelocity());
